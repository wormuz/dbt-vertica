{% macro vertica__validate_get_incremental_strategy(config) %}
  ...
  {% set invalid_strategy_msg -%}
    ...
    Expected one of: 'merge', 'delete+insert','insert_overwrite','append','update+insert'
  {%- endset %}
  {% if strategy not in ['merge', 'delete+insert','insert_overwrite','append','update+insert'] %}
...
{% macro vertica__get_incremental_sql(strategy, target_relation, tmp_relation, unique_key, dest_columns) %}
  ...
  {% elif strategy == 'update+insert' %}
    ...
    {{ return(vertica__get_incremental_update_insert_sql(arg_dict)) }}
  {% else %}
